      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full gap-6 text-center pb-10">
            <div className="w-16 h-16 bg-teal-cpm/10 dark:bg-teal-cpm/15 rounded-full flex items-center justify-center">
              <Sparkles className="w-8 h-8 text-teal-cpm" />
            </div>
            <div>
            ...
            <div className="grid grid-cols-1 gap-2 w-full max-w-sm">
              {QUICK_QUESTIONS.map((q) => (
                <button
                  key={q}
                  onClick={() => send(q)}
                  className="text-left text-sm px-4 py-2.5 rounded-lg border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 hover:bg-teal-cpm/10 dark:hover:bg-teal-cpm/15 hover:border-teal-cpm/30 hover:text-teal-cpm transition-colors"
                >
                  {q}
                </button>
              ))}
        ...
        {messages.map((msg, i) => (
          <div key={i} className={`flex gap-3 ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
            {msg.role === "assistant" && (
              <div className="w-7 h-7 bg-teal-cpm rounded-full flex items-center justify-center shrink-0 mt-0.5">
        ...
        {loading && (
          <div className="flex gap-3 justify-start">
            <div className="w-7 h-7 bg-teal-cpm rounded-full flex items-center justify-center shrink-0">
              <Bot className="w-3.5 h-3.5 text-white" />
            </div>
