  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-6 space-y-5">
      ...
      <div className={sectionCls}>
        ...
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm font-medium text-slate-900 dark:text-zinc-100">Se déconnecter</p>
            <p className="text-xs text-slate-500 dark:text-zinc-500">Fermer la session actuelle</p>
          </div>
          <Button size="sm" variant="outline" onClick={handleLogout} disabled={loggingOut}>
            <LogOut className="w-4 h-4" />
            {loggingOut ? "Déconnexion..." : "Log out"}
          </Button>
        </div>
      </div>
