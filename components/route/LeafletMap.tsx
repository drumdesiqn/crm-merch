...
export function LeafletMap({
  ...
  useEffect(() => {
    ...
    const homeIcon = L.divIcon({
      html: `<div style="background:#2563EB;color:white;border-radius:50%;width:32px;height:32px;display:flex;align-items:center;justify-content:center;font-size:16px;box-shadow:0 2px 6px rgba(0,0,0,0.3);">🏠</div>`,
      className: "",
      iconSize: [32, 32],
      iconAnchor: [16, 16],
    });
    ...
    if (routeGeometry && routeGeometry.length > 1) {
      ...
      const line = L.polyline(latLngs, { color: "#2563EB", weight: 4, opacity: 0.8 }).addTo(map);
      layersRef.current.push(line);
      map.fitBounds(L.latLngBounds(latLngs), { padding: [40, 40] });
    } else if (fallbackPoints.length > 1) {
      const line = L.polyline(fallbackPoints, { color: "#2563EB", weight: 2.5, opacity: 0.7, dashArray: "6, 6" }).addTo(map);
      layersRef.current.push(line);
